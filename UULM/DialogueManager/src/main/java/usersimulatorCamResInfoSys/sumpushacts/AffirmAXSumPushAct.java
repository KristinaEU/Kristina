...
public class AffirmAXSumPushAct extends SummaryPushAction {
	private Map<String, String> slots;

	public AffirmAXSumPushAct(double prob, SummaryPushActType t) {
		super(prob, t);
	...
	public void extractInformation(SummaryCondition sc, UserGoal g) {
		...
		if (sc instanceof ConfirmOKSumCond) {
			ConfirmOKSumCond aSC = (ConfirmOKSumCond) sc;
			Vector<String> vec = aSC.getSlots();
			for (int i = 0; i < vec.size(); i++) {
				slots.put(vec.elementAt(i), aSC.getValue(vec.elementAt(i)));
			}
		}
		else{
