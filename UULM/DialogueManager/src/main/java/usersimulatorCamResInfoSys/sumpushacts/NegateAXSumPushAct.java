...
public class NegateAXSumPushAct extends SummaryPushAction {
	private Map<String, String> slots;

	public NegateAXSumPushAct(double prob, SummaryPushActType t) {
		super(prob, t);
	...
	public void extractInformation(SummaryCondition sc, UserGoal g) {
		...
		if (sc instanceof ConfirmNotOKSumCond) {
			...
			for (Constraint c : g.getConstraints()) {
				for (String s : aSC.getSlots()) {
					if (c.getSlot().equals(s))
						addSlotValue(s, c.getValue());
				}
			}
