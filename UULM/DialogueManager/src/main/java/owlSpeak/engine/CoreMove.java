...
public class CoreMove {
	...
	/**
	 * a function that is used to increment a Variable specified by its name
	 * in which the Variable can be within a different ontology
	 * 
	 * @param var
	 *            the name of the Variable
	 * @param factory
	 ...
	 *            the name of the current user
	 */
	public static void incrementVariable(String var, OSFactory factory,
			Core core, BeliefSpace bel, String user) {
		if (var.contains(":")) {
			String[] parse;
			parse = var.split(":");
			...
			try {
				Variable vara = find.factory.getVariable(parse[1]);
				String varVal = core.getVariableString(vara, bel);
				int i = Integer.parseInt(varVal);
				String newVarVal = Integer.toString(i+1);
				core.setVariableString(vara, BS, find.factory, newVarVal, user);
			} catch (NullPointerException n) {
				n.printStackTrace();
			}
			find.write();
			...
			try {
				...
				int i = Integer.parseInt(varVal);
				String newVarVal = Integer.toString(i+1);
				core.setVariableString(vara, bel, factory, newVarVal, user);
			} catch (NullPointerException n) {
				n.printStackTrace();
				System.out
						.println("There was a problem setting the variable string in belief space "
	...
	/**
	 * a function that is used to decrement a Variable specified by its name
	 * in which the Variable can be within a different ontology
	 * 
	 * @param var
	 *            the name of the Variable
	 ...
	 *            the name of the current user
	 */
	public static void decrementVariable(String var, OSFactory factory,
			Core core, BeliefSpace bel, String user) {
		if (var.contains(":")) {
			String[] parse;
			parse = var.split(":");
