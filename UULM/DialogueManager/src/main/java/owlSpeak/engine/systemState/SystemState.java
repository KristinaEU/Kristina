...
public abstract class SystemState {
	...
	public static void processMove(UserAction userAction, BeliefSpace beliefSpace,
		...
		if (toDoMove.hasVariableOperator()) {
			...
			if (owlScript[0].equalsIgnoreCase("SET")) {
				...
				for (int i = 0; i < sets.size(); i++) {
					String[] set = owlSpeak.engine.OwlScript.parseSet(sets
							.get(i));
					int n = OwlScript.validate(set[1]);
					if (n == -42) {
						CoreMove.setVariable(set[0], set[1], factory, core,
								beliefSpace, user);
					} else {
						CoreMove.setVariable(set[0], Integer.toString(n), factory, core,
								beliefSpace, user);
					}
			...
			} else if (owlScript[0].equalsIgnoreCase("INCREMENT")) {
				String varName = owlScript[1].substring(1, owlScript[1].length()-1);
				CoreMove.incrementVariable(varName, factory, core, beliefSpace, user);
			} else if (owlScript[0].equalsIgnoreCase("DECREMENT")) {
				String varName = owlScript[1].substring(1, owlScript[1].length()-1);
				CoreMove.decrementVariable(varName, factory, core, beliefSpace, user);
			}
		}
	}
}
