...
public class AgendaPrio {
	 ...
	 */ 
	public static int calculateAgendaPrio(List<Agenda> agenda, WorkSpace workspace, OSFactory factory, Core core, int actualtime){
		float result = 0;
		for(Agenda a: agenda){		
		result += calculateAgendaPrio(a, workspace, factory, core, actualtime);;
		}
		result = result/agenda.size();
