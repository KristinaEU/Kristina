...
import owlSpeak.servlet.document.EnhancedVoiceDocument;
import owlSpeak.servlet.document.HtmlSaltDocument;
...
import owlSpeak.servlet.document.VoiceDocumentPOMDP;
...
public class ServletEngine {
	...
	public OwlDocument buildExecute(String whereAmI, String execute,
		...
		if (execute.equalsIgnoreCase("reduce")) {
			int prio = systemCore.actualOnto[Settings.getuserpos(user)].actualAgenda.get(Settings.getuserpos(user)).get(0).getPriority();
			prio = (int) (prio * 0.9);
			System.out.println("New piority: " + prio);
			Agenda muh = systemCore.actualOnto[Settings.getuserpos(user)].factory
					.getAgenda(systemCore.actualOnto[Settings.getuserpos(user)].actualAgenda.get(Settings.getuserpos(user)).get(0).getLocalName());
	...
	public String listOntos() {
		...
		for (int i = 0; i < systemCore.ontologies.size(); i++) {
			for (int j = 0; j < Settings.usersetting.length; j++) {
				String actualAgenda = "";
				if ((systemCore.ontologies.get(i).actualAgenda.get(j).get(0) != null)
						&& (systemCore.ontologies.get(i).isenabled[j])) {
					actualAgenda = (systemCore.ontologies.get(i).actualAgenda.get(j).get(0))
							.getLocalName();
				}
		...
		if (settings.getusersetting(user).sleep == 0) {
			...
			if (currentOnto != null && pureRequest) {
				...
				SystemState
						...
						.processRequestMoves(
								currentOnto.actualAgenda.get(Settings
...
//ATTENTION!!						
			...
			OwlSpeakOntology actualOnto = systemCore.actualOnto[Settings
					.getuserpos(user)];
			List<Agenda> actualAgenda = actualOnto.actualAgenda.get(Settings
					.getuserpos(user));
			try {
...
//						.getuserpos(user)]);
			...
			for(Agenda a: actualAgenda){
			Iterator<Move> neuAgenda = a.getHas().iterator();
			while (neuAgenda.hasNext()) {
				tempMove = neuAgenda.next();

				tempCoreMove = new CoreMove(tempMove, a, actualOnto,
						systemCore);
			...
			System.out.println(formatter.format(date)
					+ " Request: "
					+ actualAgenda.stream().map(a -> a.getLocalName()).collect(Collectors.toList())
					// + " (Reward/Return: "
					// + reward + "/"
			...
			OwlDocument doc = getDocByType(
					...
					noInputCounter);
			if (doc instanceof UsersimulatorDocument && Settings.ontologies[settings.getontopos(actualOnto.Name)].ontoStateVariant == SystemStateVariant.DISTRIBUTION) {
				UsersimulatorDocument uDoc = (UsersimulatorDocument) doc;
				if (actualAgenda.stream().anyMatch(a -> !a.hasSummaryAgenda()))
					System.err.println("Agenda " + actualAgenda.stream().filter(a -> !a.hasSummaryAgenda()).map(a -> a.getFullName()).collect(Collectors.toList()) + " does not belong to a summary agenda.");
				//TODO: SummaryAgenda is retrieved only for first Agenda of the reponse, change to enable all SummaryAgendas?
				uDoc.addSummaryBeliefNonPersistant(new SummaryBeliefNonpersistent(
						actualOnto.partitionDistributions[Settings
								...
								.getMove() : null, actualOnto, actualAgenda.get(0)
								.getSummaryAgenda()));
...
//					user);
		...
		if (!(agenda.equals(actualOnto.actualAgenda.get(Settings.getuserpos(user)).get(0)
				.getLocalName())))
			throw new DocumentBuildException(user);
		else
			// identify active agenda in ontology
			actualAgenda = actualOnto.actualAgenda.get(Settings.getuserpos(user)).get(0);
...
//									Settings.sysCommands[i].toLowerCase(), user);
			...
			for(Agenda a: respawnOnto.actualAgenda.get(Settings.getuserpos(user))){
			systemCore.tidyUpAgenda(
					a,
...
//			return buildRequest(whereAmI, user, null); // TODO user act??
		...
		for(Agenda a: actualOnto.actualAgenda.get(Settings.getuserpos(user))){
			systemCore.tidyUpAgenda(
					a,
					actualOnto.workSpace[Settings.getuserpos(user)],
					actualOnto.factory, false);
	...
	public OwlDocument buildSolveConflict(String whereAmI, String conflict,
		...
		return ((getDocByType(settings.getusersetting(user).docType,
				...
				.buildSolveConflict(
						utterance,
						grammar,
						systemCore.actualOnto[Settings.getuserpos(user)].actualAgenda.get(Settings
								.getuserpos(user)).get(0), whereAmI, user);
	}
	...
	public OwlDocument buildPluginAlternatives(Vector<Result> result,
		...
		return ((getDocByType(settings.getusersetting(user).docType,
				...
				.buildPluginAlternatives(result, systemCore.actualOnto[Settings
						.getuserpos(user)].actualAgenda.get(Settings
						.getuserpos(user)).get(0), whereAmI, user);
	}
...
//		return buildRequest(whereAmI, user, null, null, false);
	...
	public OwlDocument getDocByType(int docType, int grammarType) {
		...
		case Usersetting.USER_SIM_DOC:
			return new UsersimulatorDocument();
		default:
			return null;
		}
	}
