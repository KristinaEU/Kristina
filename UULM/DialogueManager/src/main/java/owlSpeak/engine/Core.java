...
//												+ ";" + temp).length() - 1)));
			...
			if (owlScript[0].equalsIgnoreCase("SET")) {
				...
				for (int i = 0; i < sets.size(); i++) {
					...
					if (n == -42) {
						...
						CoreMove.setVariable(set[0], Integer.toString(n), factory, this,
								beliefSpace, user);
					}
				}
	...
	public void matchAgenda(String user) {
		...
		while (ontos.hasNext()) {
			...
			if (tempOnto.isenabled(userpos)) {
				...
				if (tempAgenda == null)
					continue;
				List<Agenda> l = new LinkedList<Agenda>();
				l.add(tempAgenda);
				tempOnto.setActualAgenda(userpos, l);
	 ...
	 *            the name of the current user
	 */
	public static void setVariableString(Variable var, BeliefSpace bel,
