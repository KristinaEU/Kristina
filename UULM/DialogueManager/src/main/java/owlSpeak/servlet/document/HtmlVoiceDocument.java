...
public class HtmlVoiceDocument extends Document implements OwlDocument {
	...
	public void addGrammar(Element e, String grammar, String agenda, String move, String user) {
		...
		try {
			queryString += "&speak="+URLEncoder.encode(speak, "utf-8")+"&Speak0="+URLEncoder.encode(speak, "utf-8");
	...
	public void buildGrammar(Element element, Vector<CoreMove> grammar, List<Agenda> actualAgenda, String whereAmI, String user) {
		...
		while (!grammar.isEmpty()){
			...
			for(Agenda a: actualAgenda){
				this.addGrammar(add, tempCoreMv.parseGrammarVariables(tempCoreMv.onto.beliefSpace[Settings.getuserpos(user)], user), a.getLocalName(), tempCoreMv.onto.Name+"_a1n1d_"+tempMv.getLocalName(), user);
			}
			this.addBr(add);
		}
	}
	...
	@Override
	public void buildUtterance(Element element, Vector<CoreMove> utterance, List<Agenda> actualAgenda, String whereAmI, String user) {
		Element add = new Element("a");
		element.addContent(add);
	...
	@Override
	public OwlDocument fillDocument(Vector<CoreMove> utterance,Vector<CoreMove> grammar, List<Agenda> actualAgenda, String whereAmI, String user, String noInputCounter) {
		HtmlVoiceDocument VDoc = new HtmlVoiceDocument();
		...
		if(!grammar.isEmpty()) this.buildGrammar(dan, grammar, actualAgenda, whereAmI, user); else this.buildFooter(dan, whereAmI + "?com=work&agenda="+actualAgenda.get(0).getLocalName()+"&user="+user, user);
		return VDoc;
	}
