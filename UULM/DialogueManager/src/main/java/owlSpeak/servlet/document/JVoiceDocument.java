...
public class JVoiceDocument extends Document implements OwlDocument{
	...
	public JVoiceDocument fillDocument(Vector<CoreMove> utterance, Vector<CoreMove> grammar, List<Agenda> actualAgenda, String nextPath, String user, String noInputCounter){
		JVoiceDocument VDoc = new JVoiceDocument();
	
		if(   (!utterance.isEmpty())&&(grammar.isEmpty())   ){ //System Turn
    		Element dialogueForm = VDoc.newDialogueForm(actualAgenda.get(0).getLocalName(), user);
