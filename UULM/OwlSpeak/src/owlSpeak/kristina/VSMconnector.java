...
public class VSMconnector {
	...
	static KristinaDocument systemMove;
	
	public static void main(String[] args){
		try { 
			System.setProperty("owlSpeak.settings.file", "../SpokenDialogueOntologies/settings.xml");
	...
	public static void recv(DatagramPacket packet) throws IOException{
		...
		if(message.equals("ready")){
			systemMove = (KristinaDocument) (OwlSpeakServlet.processRequest(whereAmI, null, user, "0", owlEngine));
		}
		else{
			systemMove = (KristinaDocument) (OwlSpeakServlet.processWork(whereAmI, systemMove.getAgenda(), message, "1", "Hallo", user, owlEngine)); 
		}
