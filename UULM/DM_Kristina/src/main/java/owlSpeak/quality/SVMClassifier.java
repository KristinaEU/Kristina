...
public class SVMClassifier {
	...
	public SVMClassifier() {
		...
		try {
			...
			logger.logp(Level.SEVERE, "SVM Classification ", "SVMClassifier()" , "libsvm scale parameter file could not be found. " + e.getMessage());
		} catch (IOException e) {
			logger.logp(Level.SEVERE, "SVM Classification ", "SVMClassifier()" , "Problems with reading the scale parameters occurred. "	+ e.getMessage());
		}
		...
		try {
			...
			logger.logp(Level.SEVERE, "SVM Classification ", "SVMClassifier()" , "libsvm model file could not be found.", e);
		} catch (IOException e) {
			logger.logp(Level.SEVERE, "SVM Classification ", "SVMClassifier()" , "Problems with reading the libsvm model occured.",e);
		}
	}
	...
	SVMResult evaluate(QualityParameter data) throws Exception {
		...
		if (getProbability) {
			if (svm.svm_check_probability_model(model) == 0) {
				logger.logp(Level.WARNING, "SVM Classification", "evaluate(QualityParameter)" , "Model does not support probabiliy estimates");
